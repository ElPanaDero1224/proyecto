...
<div class="container mt-4">
 ...
 <div class="text-center mb-4">
        <h1 class="display-4">Editar Hotel</h1>
                ...
                <div class="col-md-8">
                    <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="nombre" value="{{ old('nombre', $hotel->nombre) }}">
                                <small>{{ $errors->first("nombre") }}</small>
                            </div>

                            <div class="form-group">
                                <label for="ubicacion"><strong>Ubicación:</strong></label>
                                <input type="text" class="form-control" name="ubicacion" value="{{ old('ubicacion', $hotel->ubicacion) }}">
    ...
    <select class="form-control" name="categoria">
        ...
        @for ($i = 1; $i <= 5; $i++)
            <option value="{{ $i }}" 
                {{ old('categoria', $hotel->categoria) == $i ? 'selected' : '' }}>
                {{ $i }}
            </option>
        @endfor
    </select>
    <small>{{ $errors->first("categoria") }}</small>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="descripcion" rows="3">{{ old('descripcion', $hotel->descripcion) }}</textarea>
                                <small>{{ $errors->first("descripcion") }}</small>
                            </div>

                            <div class="form-group">
                                <label for="servicios"><strong>Servicios:</strong></label>
                                <textarea class="form-control" name="servicios" rows="3">{{ old('servicios', $hotel->servicios) }}</textarea>
                                <small>{{ $errors->first("servicios") }}</small>
                            </div>

                            <div class="form-group">
                                <label for="distancia_puntos_turisticos"><strong>Distancia a puntos turísticos:</strong></label>
                                <textarea class="form-control" name="distancia_puntos_turisticos" rows="3">{{ old('distancia_puntos_turisticos', $hotel->distancia_puntos_turisticos) }}</textarea>
                                <small>{{ $errors->first("distancia_puntos_turisticos") }}</small>
                            </div>

                            <div class="form-group">
                                <label for="distancia_centro"><strong>Distancia al centro (km):</strong></label>
                                <input type="text" class="form-control" name="distancia_centro" value="{{ old('distancia_centro', $hotel->distancia_centro) }}">
                                <small>{{ $errors->first("distancia_centro") }}</small>
                            </div>

                            <div class="form-group">
                                <label for="politicas_cancelacion"><strong>Políticas de cancelación:</strong></label>
                                <textarea class="form-control" name="politicas_cancelacion" rows="3">{{ old('politicas_cancelacion', $hotel->politicas_cancelacion) }}</textarea>
